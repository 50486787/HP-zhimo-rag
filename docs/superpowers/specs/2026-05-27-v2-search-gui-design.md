...
- `[启动 Web 服务]` `[停止]` 按钮
- 状态指示：● 运行中 / ● 已停止
- 服务 URL 显示（`http://0.0.0.0:8088`）
- `[打开浏览器]` 按钮（`os.startfile("http://127.0.0.1:8088")`）
- 服务日志：只读 Text 区域，实时显示 uvicorn 输出

Web 服务通过 `subprocess.Popen` 启动 uvicorn，stdout/stderr 管道读取到日志区。停止时发送 `terminate()`。
