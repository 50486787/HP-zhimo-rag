...
"""
    ...
    @staticmethod
    def _cache_dir():
        d = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".embed_cache")
        os.makedirs(d, exist_ok=True)
        return d

    def initialize(self):
        """加载文档 + bge-m3 embed + normalize → 存内存（自动缓存到磁盘）"""
        t0 = time.time()
        print("加载 RAGFlow 文档...")
        ...
        # 计算指纹（文档数 + 内容哈希），用于判断缓存是否有效
        fp = hashlib.md5("".join(doc_texts).encode()).hexdigest()[:12]
        cache_dir = self._cache_dir()
        npz_path = os.path.join(cache_dir, "matrix.npz")
        ...
        if os.path.isfile(npz_path) and os.path.isfile(meta_path):
            try:
                ...
                if meta.get("fp") == fp and meta.get("n") == len(doc_texts):
                    print(f"从缓存加载 embedding ({len(doc_texts)} 条)...")
                    self.doc_matrix = np.load(npz_path)["m"]
                    cache_hit = True
            except Exception:
                pass
        ...
        if not cache_hit:
            ...
            # 写入缓存
            np.savez_compressed(npz_path, m=self.doc_matrix)
            json.dump({"fp": fp, "n": len(doc_texts)}, open(meta_path, "w", encoding="utf-8"))
